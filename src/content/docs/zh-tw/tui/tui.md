...
在 type 群組標題或物件上按 `v` 進入 **view 模式**。三欄佈局會被全寬表格取代，顯示物件及其屬性欄位。
...
表格顯示物件名稱及屬性欄位（釘選屬性優先）。欄位數量會依終端機寬度自動調整。

開啟 **預覽**（`p`）時，右側面板顯示游標所在物件的屬性和內文摘要。上下移動時預覽會自動跟隨更新。
