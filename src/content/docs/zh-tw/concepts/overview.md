...
Vault 是由 TypeMD 管理的目錄。它包含 `.typemd/` 設定資料夾、`objects/` 內容資料夾，以及選填的 `templates/` object template 資料夾。每個 Vault 都是獨立的——你可以像一般資料夾一樣搬移、複製或用版本控制管理。
...
Property（屬性）是 Object 上的具名欄位，由 Type schema 定義。屬性有型別，例如 `string`、`number`、`date`、`select` 或 `relation`，儲存在 Markdown 檔案的 YAML frontmatter 中。屬性也可以有可選的附加設定，如 `emoji`（視覺圖示）、`pin`（突出顯示）和 `default`（預設值）。
...
Wiki-link 是在 Markdown 內文中使用 `[[type/slug-ulid]]` 語法來引用其他 Object 的方式。連結目標必須是完整的 Object ID（包含 ULID 後綴），例如 `[[book/golang-in-action-01jqr3k5mpbvn8e0f2g7h9txyz]]`。TypeMD 會追蹤這些連結及其反向連結（backlink），讓你發現 Object 之間的關聯。
